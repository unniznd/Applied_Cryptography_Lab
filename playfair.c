...
void decrypt(char key_matrix[6][6], char *plaintext, char *ciphertext, int len){
    ...
    for(int i = 0; i<len; i+=2){
        ...
        if(pos0[0] == pos1[0]){
            ciphertext[i] = key_matrix[pos1[0]][((pos0[1] + 5) % 6)];
            ciphertext[i+1] = key_matrix[pos1[0]][((pos1[1] + 5) % 6)];
        }
        else if(pos0[1] == pos1[1]){
            ciphertext[i] = key_matrix[((pos0[0] + 5) % 6)][pos1[1]];
            ciphertext[i+1] = key_matrix[((pos1[0] + 5) % 6)][pos1[1]];
        }
        else{
            ciphertext[i] = key_matrix[pos0[0]][pos1[1]];
            ciphertext[i+1] = key_matrix[pos1[0]][pos0[1]];
